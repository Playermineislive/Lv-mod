...
import okhttp3.OkHttpClient;
 ...
 public class MainActivity extends BaseActivity {
    ...
    private void showResolutionDialog() {
        SharedPreferences prefs = getSharedPreferences("LauncherPrefs", MODE_PRIVATE);
        float currentScale = prefs.getFloat("render_scale", 1.0f);

        final String[] options = {"Native (1080p)", "Balanced (720p)", "Smooth (540p)", "Potato (360p)"};
        final float[] values = {1.0f, 0.75f, 0.5f, 0.35f};

        int selectedIndex = 0;
        if (currentScale == 0.75f) selectedIndex = 1;
        else if (currentScale == 0.5f) selectedIndex = 2;
        else if (currentScale == 0.35f) selectedIndex = 3;

        new androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("Resolution Scaler")
            .setSingleChoiceItems(options, selectedIndex, (dialog, which) -> {
                prefs.edit().putFloat("render_scale", values[which]).apply();
