...
class MinecraftActivity : MainActivity() {
    ...
    private fun applyResolutionScale() {
        // 1. Get the saved scale value from MainActivity
        ...
        // 2. Only resize if scale is less than 100% (1.0)
        if (scale < 1.0f) {
            ...
            surfaceView?.holder?.let { holder ->
                ...
                // 3. Calculate the target resolution
                val targetWidth = (metrics.widthPixels * scale).toInt()
                val targetHeight = (metrics.heightPixels * scale).toInt()

                // 4. Force the game to render at lower resolution
                holder.setFixedSize(targetWidth, targetHeight)
                
                android.util.Log.d("LeviLauncher", "Resolution set to: ${targetWidth}x${targetHeight}")
            }
        }
    }
