...
class MinecraftActivity : MainActivity() {
    ...
    // =============================================================
    //  NUCLEAR RESOLUTION SCALER (REPEATER & ENFORCER)
    // =============================================================

    override fun onWindowFocusChanged(hasFocus: Boolean) {
        super.onWindowFocusChanged(hasFocus)
        if (hasFocus) {
            // Start the repeater loop to fight the game's loading screen reset
            attempts = 0
            startResolutionRepeater()
        }
    }

    private fun startResolutionRepeater() {
        // Run this check every 2 seconds for the first 10 seconds
        val checkRunnable = object : Runnable {
            override fun run() {
                if (attempts < 5) { // 5 checks * 2000ms = 10 seconds coverage
        ...
        resolutionHandler.post(checkRunnable)
    }
    ...
    private fun applyResolutionScale() {
        try {
            ...
            if (surfaceView != null) {
                ...
                Log.e("LeviResolution", "Applying Resolution: ${targetWidth}x${targetHeight} (Scale: $scale)")

                runOnUiThread {
                    // --- METHOD 1: Hardware Buffer ---
                    // This is the most important one for FPS
                    surfaceView.holder.setFixedSize(targetWidth, targetHeight)

                    // --- METHOD 2: Physical View Size ---
                    // Force the layout to shrink.
                    ...
                    if (resolutionEnforcer == null) {
                        resolutionEnforcer = object : SurfaceHolder.Callback {
                            ...
                            override fun surfaceChanged(h: SurfaceHolder, format: Int, w: Int, hInt: Int) {
                                // If the game resets the size (w != targetWidth), we force it back
                                if (w > targetWidth + 10) { // Tolerance of 10px
                                    Log.e("LeviResolution", "Game reset resolution to $w! Fighting back...")
                                    runOnUiThread {
                                        surfaceView.holder.setFixedSize(targetWidth, targetHeight)
                    ...
                    if (attempts == 0) {
                        Toast.makeText(this, "🚀 Forced: ${targetWidth}x${targetHeight}", Toast.LENGTH_SHORT).show()
                    }
                }
            } else {
                Log.e("LeviResolution", "Could not find SurfaceView!")
            }
        } catch (e: Exception) {
            Log.e("LeviResolution", "Error: ${e.message}")
        }
    }
